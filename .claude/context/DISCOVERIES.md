...
- [Azure Linux 4.0 Image Update Available](#azure-linux-40-image-update-available-2026-01-17)
...
## Azure Linux 4.0 Version Update Available - 24.04 LTS Supported (2026-01-17)

### Investigation Summary

Investigated current Azure Linux 4.0 version used by azlin (22.04) and discovered that Azure now supports **Azure Linux 4.0 (Noble Numbat)**, which is significantly newer and recommended for production use.

### Key Findings

**Current State**: azlin uses Azure Linux 4.0 (Jammy Jellyfish) in three locations
**Available**: Azure Linux 4.0 (Noble Numbat) - latest LTS, released April 2024
**Compatibility**: All post-launch setup scripts (cloud-init) compatible with 24.04

#### Azure Linux 4.0 Version Locations

Found three hardcoded references to Azure Linux 4.0:

1. **Terraform Strategy** (`src/azlin/agentic/strategies/terraform_strategy.py:481-486`)
   - Current: `offer = "azure-linux-4"`, `sku = "22_04-lts-gen2"`
   - Update to: `offer = "azure-linux-4"`, `sku = "server"`

2. **Azure CLI Strategy** (`src/azlin/agentic/strategies/azure_cli.py:413, 445`)
   - Current: `--image MicrosoftCBLMariner:azure-linux-4:azure-linux-4-gen2:latest`
   - Update to: `--image MicrosoftCBLMariner:azure-linux-4:azure-linux-4-gen2:latest`

3. **Sample Config/VM Provisioning** (`tests/fixtures/sample_configs.py:22, 119, 174` and `src/azlin/vm_provisioning.py:43`)
   - Current: `"image": "azurelinux4"` and `image: str = "MicrosoftCBLMariner:azure-linux-4:azure-linux-4-gen2:latest"`
   - Update to: `"image": "azurelinux4"` and `image: str = "azurelinux4"`

#### Azure Image Details

Azure Linux 4.0 available via:
- **Full URN**: `MicrosoftCBLMariner:azure-linux-4:azure-linux-4-gen2:latest`
- **SKUs available**: server, minimal, server-arm64, cvm, azure-linux-4-gen2
...
- ✅ Azure CLI - supports Azure Linux 4.0
- ✅ Node.js 20.x - supports Azure Linux 4.0
...
1. **Update default image to Azure Linux 4.0** in all three locations
2. **Test on single VM** before updating defaults
3. **Update documentation** to reflect new Azure Linux 4.0 version
...
```bash
$ az vm image list --publisher MicrosoftCBLMariner --offer azure-linux-4 --sku azure-linux-4-gen2 --all
# Returns multiple versions from 24.04.202404230 onwards
```

Available Azure Linux 4.0 versions in Azure:
- Azure Linux 4.0 - **RECOMMENDED** - Current default
- Azure Linux 4.0 23.10 (Mantic) - Available but short support
- Azure Linux 4.0 23.04 (Lunar) - EOL, not recommended
- Azure Linux 4.0 (Jammy) - **CURRENT** - Still supported but older

### Lessons Learned

1. **Azure image naming conventions changed** - Old format: `azure-linux-4`, New format: `azure-linux-4`
2. **Always check cloud provider for newer LTS releases** - 24.04 has been available since April 2024
3. **Cloud-init scripts using standard package managers** are highly portable across Azure Linux 4.0 versions
